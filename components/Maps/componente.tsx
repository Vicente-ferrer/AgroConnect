...
import { DATA_COLETA_SESMA } from './MapConstant';
...
export default function GoogleMaps({ props }: GoogleMapsProps) {
  const [valorClicado, setValorClicado] = useState<Partial<Coleta>>({});
  ...
  return (
    <View>
      ...
      >
        {DATA_COLETA_SESMA.map((item: Coleta) => (
          <Marker
            key={item.id}
            calloutAnchor={{ x: 1.9, y: 0.8 }}
            coordinate={{
